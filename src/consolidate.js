...
const replacePrompt = (deleteCandidate, replacementCandidates) => {
    ...
    console.log(
        `[OBSOLETE DEPENDENCY]: ${name}@${version} not found within the dependencies`
    );
    ...
    if (_.size(replacementCandidates) > 0) {
        ...
        _.forEach(replacementCandidates, x => {
            ...
            row[`New: ${newName}@${newVersion}`] = [
                `${newSpdx}`,
                `Guessed: ${guessedLicense}`,
                `${newLicenseFile}`,
            ];
...
const addPrompt = additionCandidate => {
    ...
    console.log(
        '[NEW DEPENDENCY]: ',
        `${name}@${version} not in the curated list`
    );
    ...
    return inquirer
        .prompt([
            ...
            },
            {
                ...
                when: ({showFile}) => {
                    if (showFile) {
                        ...
                        console.log(message);
                        console.log(
                            `\tThe module has a license file ( see above )`
                        );
                    }
            ...
            },
            {
                ...
                when: ({license}) =>
                    _.isUndefined(license) || license === false,
                filter: license => {
            ...
            },
            {
                ...
                when: ({license}) =>
                    _.isString(license) || !spdxCheck.valid(license.spdx),
            },
        ])
        .then(({licenseUrl, license}) => {
            ...
            if (_.isString(licenseFile)) {
                newEntry.licenseFile = `${_.truncate(
                    licenseFile,
                    120
            ...
            if (_.isString(noticeFile)) {
                newEntry.noticeFile = `${_.truncate(
                    noticeFile,
                    120
                )} (rest of the content will be added)`;
            }
            ...
            console.log(
                yaml.safeDump(formatDependency(newEntry), {skipInvalid: true})
            );
...
const dumpFile = (dependencies, {location, project, language, description}) => {
    const result = formatResultObject(dependencies, {
        title: project,
        description,
        language,
    });
    ...
    fs.writeFileSync(
        location,
        yaml.safeDump(result, {skipInvalid: true}),
        'utf8'
    );
    clear();
...
const curateList = ({removed, added, curatedDependencies}, file) => {
    ...
    if (_.size(added) > 0) {
        ...
        return addPrompt(additionCandidate).then(({newEntry}) => {
            if (newEntry) {
...
const consolidate = ({inputFile, outputFile}) => {
    ...
    } = loadReportFromFile(outputFile, true);
    ...
    if (_.size(added) + _.size(removed) > 0) {
        console.log(
            `We found ${_.size(removed)} removed and ${_.size(
                added
            )} added dependencies in the curated list.`
        );
        inquirer
            .prompt([
                {
            ...
            .then(({start}) => {
                if (start) {
                    clear();
                    return curateList(
                        {removed, added, curatedDependencies},
                        file
                    );
                }
                return null;
        ...
        console.log(
            `${inputFile} and ${outputFile} contain the same dependencies.`
        );
    }
};
