...
const addPrompt = (additionCandidate) => {
    ...
    const {name, version, url, noticeFile, guessedLicense = '---', spdx = '---', licenseFile = false, licenseFilePath} = additionCandidate;
    ...
    return inquirer.prompt([
        {
            type: 'confirm',
            name: 'showFile',
            message: `\tThe module has a license file ( ${licenseFilePath} )\nDo you want to see the license file now?`,
        ...
        },
        {
            ...
            when: ({showFile}) => {
                if (showFile) {
                    ...
                    console.log(message);
                    console.log(`\tThe module has a license file ( ${licenseFilePath} )`);
                }
        ...
        .then(({licenseUrl, license}) => {
            ...
            console.log(yaml.dump(newEntry));
...
const dumpFile = (dependencies, {location, project, language, description}) => {
    ...
    fs.writeFileSync(location, yaml.dump(result), 'utf8');
    clear();
...
const consolidate = ({inputFile, outputFile}) => {
    ...
    const {project: iProject, language: iLanguage, description: iDesc, dependencies: currDependencies} = loadReport(inputFile);

    const {project: oProject, language: oLanguage, description: oDesc, dependencies: curatedDependencies} = loadReport(outputFile, true);
